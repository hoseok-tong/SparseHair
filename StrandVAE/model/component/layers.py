...
from StrandVAE.util.transforms import matrix_to_rotation_6d, rotation_6d_to_matrix, axis_angle_to_matrix, matrix_to_axis_angle
...
class ModulatedSiren(nn.Module):
    def __init__(self, dim_in=1, dim_hidden=256, dim_out=3, num_layers=5, w0_initial=30., latent_dim=64, coord_length=100):
        super().__init__()
        self.net = SirenNet(dim_in=dim_in,
                            dim_hidden=dim_hidden,
                            dim_out=dim_out,
                            num_layers=num_layers,
                            w0_initial=w0_initial)
        self.modulator = Modulator(dim_in=latent_dim,
                                   dim_hidden=self.net.dim_hidden,
                                   num_layers=self.net.num_layers)

        tensors = [torch.linspace(0, 1, steps=coord_length)]    # relative position along the strand from root (0) to tip (1)
        mgrid = torch.stack(tensors, dim=-1)
        self.register_buffer('grid', mgrid)
    ...
    def forward(self, latent):
        ...
        coords = self.grid.clone().detach().requires_grad_()
        coords = repeat(coords, 'n d -> b n d', b=latent.shape[0])

        y = self.net(coords, mods)

        if y.shape[2] == 6:
            return self.direction_to_point(matrix_to_axis_angle(rotation_6d_to_matrix(y)))
        else:
            p = self.direction_to_point(y)
            return p

    @staticmethod
    def direction_to_point(d):
        p = [d[:, 0:1] - d[:, 0:1]]  # Start from origin (zeros)
        for idx in range(1, d.shape[1]):  # Start from idx = 0
            p += [p[-1] + d[:, idx:idx+1]]
        p = torch.cat(p, dim=1)
        return p
